...
/*
	private double calculateXForFanSpeed(double yValue,double[] point1,double[] point2)
	{

		double m=(point2[1]-point1[1])/(point2[0]-point1[0]);
		double q=point1[1]-m*point1[0];
		double newX=(yValue-q)/m;
	...
	private double getValueForFanSpeed(double[] highest)
	{
		double xRetunValue=-1;
		int type=(int)highest[0];
		double value=highest[1];
		System.out.println(type+"->"+highest[1]);


		double[][] fanParams=new double[2][];
		fanParams[0]=manager.getFanSpeed().getFS();

		if(type==0){fanParams[1]=manager.getFanSpeed().getLOW();}
		if(type==1){fanParams[1]=manager.getFanSpeed().getMED();}
		if(type==2){fanParams[1]=manager.getFanSpeed().getHIGH();}


		for(int i=0;i<fanParams[0].length-1;i++)
		{
			//System.out.println("-------------------------type>"+type+" pos:"+i+"->"+value+" "+fanParams[1][i]+"<->"+fanParams[1][i+1]);
			if(value==fanParams[1][i])
			{
				return fanParams[0][i];
			}
			else if(value==fanParams[1][i+1])
			{
				return fanParams[0][i+1];
			}
			else if((value>fanParams[1][i] && value<fanParams[1][i+1]) ||(value>fanParams[1][i+1] && value<fanParams[1][i]))
			{

				return calculateXForFanSpeed(value, new double[]{fanParams[0][i],fanParams[1][i]}, new double[]{fanParams[0][i+1],fanParams[1][i+1]});
			}
		}
	...
	private double[] getHighest(double low,double med,double high)
	{
		int highestPos=0;
		double highest=low;
		if(med>highest)
		...
		return new double[]{highestPos,highest};
